...
public class ApiGen {
    ...
    private static final String GUICE_MODULE = "GuiceModule";
    public static final String GUICE_MODULE_CAMEL_CASE = "guiceModule";

    private final Parser parser = new Parser();
    private final Path outputDirectory;
    private final STGroup stGroup;
    private final String guiceModuleName;
    private final String defaultPackageName;
    private final Map<String, TypeEntry> generatedTypes = new LinkedHashMap<>();
    private final Map<String, TypeEntry> referenceTypes = new HashMap<>();
    private final List<TypeEntry> schemaDefinitions = new ArrayList<>();
    ...
    private void add(Map<String, TypeEntry> types, URL path) throws IOException {
        ...
        try {
            ...
            for (Definition<?> definition : doc.getDefinitions()) {
                if (definition instanceof SchemaDefinition) {
                    ...
                    throw new RuntimeException(
                            String.format("GraphQL schema documents must only contain schema type definitions, got %s [%s, %s]",
                ...
                if (null != oldEntry) {
                    ...
                    throw new RuntimeException(
                            String.format("Duplicate type definition for '%s' defined both in '%s' and '%s'.",
                                    newEntry.getName(), oldEntry.getSourceLocation(), newEntry.getSourceLocation()));
                }
            ...
            throw new RuntimeException(String.format("%s when parsing '%s'", ex.getMessage(), path), ex);
        }
    }
    ...
    public void generate() throws IOException {
        Set<String> generatorNames = new TreeSet<>();

        for (String name : stGroup.getTemplateNames()) {
            if (!name.endsWith(FILE_NAME)) continue;
            String generatorName = name.substring(0, name.length() - FILE_NAME.length());
        ...
        for (TypeEntry entry : allEntries) {
            try {
                ...
                for (String generatorName : generatorNames) {
                    String fileName = stGroup.getInstanceOf(generatorName + FILE_NAME)
                            .add("model", model)
                            .render();
                    ...
                    String content = stGroup.getInstanceOf(generatorName + GENERATOR)
                            .add("model", model)
                            .render();
                    if (stGroup.isDefined(generatorName + GUICE_MODULE)) {
                        moduleBuilder.append(stGroup.getInstanceOf(generatorName + GUICE_MODULE)
                                .add("model", model)
                                .render());
                    }
        ...
        if (moduleBuilder.length() > 0 && guiceModuleName != null && stGroup.isDefined(GUICE_MODULE_CAMEL_CASE)) {
            PackageClassName packageClassName = getPackageClassName(guiceModuleName);
            String content = stGroup.getInstanceOf(GUICE_MODULE_CAMEL_CASE)
    ...
    private STGroup getDefaultSTGroup() {
